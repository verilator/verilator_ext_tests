...
setenv('RV_ROOT', File::Spec->rel2abs($Self->{t_dir}."/../submodules/Cores-VeeR-EL2"));
...
# Note the build happens in $Self->{obj_dir} as the VeeR build system can
# find everything via RV_ROOT. This leaves the submodule clean.
$Self->skip("cores-VeeR-EL2 has unfixed mason dependency");
